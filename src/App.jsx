...
function Onboarding({ initialProfile, onFinish, onCancel }) {
  ...
  return <form className="simple-onboard" onSubmit={(event) => { event.preventDefault(); next() }}><div className="on-progress"><i style={{ width: `${((step + 1) / 4) * 100}%` }} /></div>
    {step === 0 && <section className="on-welcome"><img className="on-logo" src={`${import.meta.env.BASE_URL}art/logo_lockup.png`} alt="My Pace — Small Steps, Big Changes" /><div className="on-mascot"><Mascot mood="idle" size="lg" /></div><h1>작은 오늘이,<br />더 나은 나를 만들어요.</h1><p>완벽하지 않아도 괜찮아요.<br />내 페이스로, 조금씩.</p></section>}
