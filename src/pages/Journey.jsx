...
export default function Journey({ totalMeters, history, legacySpecial, weeklySpecials, now, discoveredIds, actualDiscoveries, discoveryRecords = [] }) {
  ...
  if (view === 'detail' && selectedMilestone) {
    ...
    return <main className="screen journey-screen milestone-detail-screen"><section className="collection-head detail-head"><button className="icon-back" onClick={() => setView('collection')} aria-label="나의 발자취로 돌아가기"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14.5 5-7 7 7 7" /></svg></button><div><small>{selectedMilestone.type === 'achievement' ? 'PERSONAL MILESTONE' : 'JOURNEY LANDMARK'}</small><h1>{selectedMilestone.name}</h1><p>{selectedMilestone.region}</p></div></section><section className="screen-content milestone-detail"><span className="detail-icon"><MilestoneIcon milestone={selectedMilestone} className="milestone-icon-xl" /></span><strong>{distance(selectedMilestone.meters)}</strong><p className="detail-description">{selectedMilestone.description}</p><article><small>발견한 순간</small><b>{record?.totalMeters ? distance(record.totalMeters) : distance(selectedMilestone.meters)}</b><p>{formatDiscoveryDate(record?.discoveredAt)}</p></article><article><small>Daily Goal로 환산하면</small><b>약 {dailyGoalCount(selectedMilestone.meters)}개</b><p>Daily Goal 1회 = 20m 기준의 단순 환산이에요.</p></article><aside><b>💡 잠깐 상식</b><p>{selectedMilestone.fact}</p></aside></section></main>
  }

  if (view === 'collection') return <main className="screen journey-screen collection-screen"><section className="collection-head"><button className="icon-back" onClick={() => setView('main')} aria-label="여정으로 돌아가기"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m14.5 5-7 7 7 7" /></svg></button><div><small>JOURNEY COLLECTION</small><h1>나의 발자취</h1><p>{discovered.length}개 발견</p></div></section><section className="screen-content collection-grid">{discovered.map((item) => <button key={item.id} className="collection-card unlocked" onClick={() => { setSelectedMilestone(item); setView('detail') }}><span><MilestoneIcon milestone={item} className="milestone-icon-md" /></span><small>{item.type === 'achievement' ? '나의 성취' : item.region}</small><b>{item.name}</b><em>{distance(item.meters)}</em><p>{item.description}</p></button>)}{next && <article className="collection-card locked next-card"><span>?</span><small>다음 Discovery</small><b>???</b><em>{distance(next.meters - totalMeters)} 남음</em><p>Daily Goal 기준 약 {dailyGoalCount(next.meters - totalMeters)}개 남았어요.</p></article>}</section></main>
